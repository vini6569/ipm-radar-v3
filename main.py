...
def processar_live():
    ...
    # CORRECAO PRINCIPAL:
    # mesmo se /events/live nao devolver os jogos, consultamos as odds
    # diretamente pelos IDs monitorados. Assim a movimentacao da ODD X
    # nao fica dependente de a primeira consulta live casar os IDs.
...
def formatar_pre_entrada(dados):
    return (
        ...
        "LABORATORIO IPM\n"
        "Sinal estatistico para observacao. Nao realiza apostas."
    )
...
def montar_mensagens(resultados):
    ...
    for jogo in resultados:
        ...
        linhas.extend([
            ...
            f"Q: {q:.2f}", f"R: {r:.2f}",
            f"Equilibrio: {equilibrio:.2f}%",
            f"Desequilibrio: {desequilibrio:.2f}%",
        ...
        if len("\n".join(linhas)) > TELEGRAM_MAX_CARACTERES:
            ...
            linhas.extend([
                ...
                f"Casa {odd_casa:.2f} | X {odd_empate:.2f} | Visitante {odd_visitante:.2f}",
                f"Q: {q:.2f}", f"R: {r:.2f}",
                f"Equilibrio: {equilibrio:.2f}%", f"Desequilibrio: {desequilibrio:.2f}%",
                f"Estrutura: {classificacao or 'NAO CLASSIFICADO'}",
                f"Padrao: {padrao or 'NAO CLASSIFICADO'}",
                f"P(X): {prob_x:.2f}% | P(X) N: {prob_x_normalizada:.2f}%", "",
            ])
    if len(linhas) > 7:
        linhas.extend(["--------------------", f"Jogos selecionados: {len(resultados)}", "", "IPM-RADAR-V5", "Monitoramento estatistico pre-live.", "Nao realiza apostas automaticamente."])
        mensagens.append("\n".join(linhas))
    return mensagens
...
def loop_consulta():
    ...
    while True:
        ...
        print(f"CICLO TERMINADO EM {agora - inicio:.1f}s | PROXIMO CICLO EM {espera:.0f}s")
        time.sleep(espera)
