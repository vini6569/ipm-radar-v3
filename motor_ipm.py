...
def analisar_ipm_com_memoria(
    ...
    **kwargs,
    ...
    if memoria["odd_visitante_inicial"] is None and odd_visitante > 0:
    ...
    odd_45 = _calcular_referencia_45(odd_casa, odd_empate, odd_visitante, minuto)
    diferenca_45 = 0.0
...
def avaliar_entrada(resultado, minuto, ipm_minimo, variacao_minima, minuto_minimo, minuto_maximo):
...
def jogo_finalizado(jogo):
    ...
    finais = ("finished", "ft", "final", "ended", "complete", "completed")
    return status in finais or status.startswith("finished") or status.startswith("ended")
...
def formatar_radar(jogo, resultado, mercados=None):
    ...
    return (
        ...
        "💰 ODDS\n"
        f"🏠 Casa:      {_numero(resultado.get('odd_casa')):.2f} ({_numero(resultado.get('variacao_casa')):+.2f}%)\n"
        f"🤝 Empate:    {_numero(resultado.get('odd_empate')):.2f} ({_numero(resultado.get('variacao_empate')):+.2f}%)\n"
        f"🚌 Visitante: {_numero(resultado.get('odd_visitante')):.2f} ({_numero(resultado.get('variacao_visitante')):+.2f}%)\n\n"
        "🎯 REFERÊNCIA 45'\n"
