...
def classificar_forca(variacao_odd):
    try:
        ...
        return "ESTAVEL"
    if valor >= 10: return "MUITO FORTE"
    if valor >= 5: return "FORTE"
...
def classificar_direcao(variacao_odd):
    try:
        ...
        return "ESTAVEL"
    if valor < -0.05: return "QUEDA"
    if valor > 0.05: return "ALTA"
    return "ESTAVEL"

def calcular_ipm(variacao_odd, minuto=0, gols=0, escanteios=0,
                 finalizacoes=0, ataques_perigosos=0):
    try:
        ...
        ipm = min(100.0, movimento + confirmacao_gols +
                  confirmacao_escanteios + confirmacao_finalizacoes +
                  confirmacao_ataques)
        ...
        return {
            ...
            "minuto": minuto, "gols": gols, "escanteios": escanteios,
            "finalizacoes": finalizacoes,
            "ataques_perigosos": ataques_perigosos,
        }
    except Exception as erro:
        return {"ipm": 0.0, "variacao_odd": 0.0, "forca": "ESTAVEL",
                "direcao": "ESTAVEL", "minuto": minuto, "gols": gols,
                "escanteios": escanteios, "finalizacoes": finalizacoes,
                "ataques_perigosos": ataques_perigosos, "erro": str(erro)}

def analisar_ipm_com_memoria(chave_jogo, odd_atual, minuto=0, gols=0,
                             escanteios=0, finalizacoes=0,
                             ataques_perigosos=0):
    ...
    atual = _converter_odd(odd_atual)
    if atual is None:
        resultado = calcular_ipm(0.0, minuto, gols, escanteios,
                                 finalizacoes, ataques_perigosos)
        resultado.update({"odd_anterior": None, "odd_atual": odd_atual,
    ...
    resultado = calcular_ipm(variacao, minuto, gols, escanteios,
                             finalizacoes, ataques_perigosos)
    resultado.update({"odd_anterior": anterior, "odd_atual": atual,
...
def _placar(jogo, mercados=None):
    ...
    for fonte in (jogo, mercados):
        for chave in ("score", "scores", "placar"):
            v = fonte.get(chave)
            if isinstance(v, dict):
                a = v.get("home", v.get("home_score"))
                b = v.get("away", v.get("away_score"))
            ...
            if isinstance(v, str) and "x" in v.lower():
                try:
                    a, b = v.lower().replace(" ", "").split("x", 1)
                    return int(a), int(b)
                except (TypeError, ValueError): pass
    try:
        return int(jogo["home_score"]), int(jogo["away_score"])
    except (KeyError, TypeError, ValueError):
        return None, None

def obter_status_jogo(jogo):
    if not isinstance(jogo, dict): return ""
    for chave in ("status", "state", "match_status"):
        v = jogo.get(chave)
        if isinstance(v, dict):
            for sub in ("short", "long", "name", "status"):
                if v.get(sub): return str(v[sub]).upper()
        elif v is not None:
            return str(v).upper()
    return ""
...
def jogo_finalizado(jogo):
    ...
    return any(x in status for x in
               ("FINISHED", "FT", "FINALIZADO", "ENCERRADO", "AFTER"))

def resultado_empate(jogo, mercados=None):
    a, b = _placar(jogo, mercados)
    return None if a is None or b is None else a == b

def avaliar_entrada(resultado, minuto, ipm_minimo=40.0,
                    variacao_minima=0.5, minuto_minimo=1,
                    minuto_maximo=5):
    try:
        m, ipm, var = int(minuto), float(resultado.get("ipm", 0)), abs(float(resultado.get("variacao_odd", 0)))
    except (TypeError, ValueError):
        return False
    return (minuto_minimo <= m <= minuto_maximo
            and not resultado.get("primeira_consulta", False)
            and ipm >= ipm_minimo and var >= variacao_minima)
...
def formatar_radar(jogo, resultado, mercados=None):
    ...
    var = "AGUARDANDO COMPARAÇÃO" if resultado.get("primeira_consulta") else f'{resultado.get("variacao_odd", 0):+.2f}%'
    return "\n".join([
        "", "=" * 70, "📡 IPM RADAR V4.2", "=" * 70,
        f"⚽ {casa} x {fora}", f"⏱️ Minuto: {resultado.get('minuto', 0)}",
        f"📊 Placar: {placar}", f"💰 Odd empate anterior: {resultado.get('odd_anterior')}",
        f"💰 Odd empate atual: {resultado.get('odd_atual')}", f"📈 Variação: {var}",
