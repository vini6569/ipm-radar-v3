...
def montar_mensagens(resultados):
    ...
    for jogo in resultados:
        ...
        fora = numero(jogo.get("odd_visitante"))
        q = numero(jogo.get("q"))
        r = numero(jogo.get("r"))
        prob = numero(jogo.get("probabilidade_x"))
        prob_n = numero(jogo.get("probabilidade_x_normalizada"))
    ...
    while True:
        ...
        try:
            if horario_ativo():
                ...
                print("PRE-LIVE | Periodo de pausa.")
        except Exception as erro:
            print(
                "ERRO PRE-LIVE:",
                type(erro).__name__,
