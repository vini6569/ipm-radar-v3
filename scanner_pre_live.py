...
def escanear_pre_live():
    ...
    print(f"Q MÍNIMO ENCONTRADO: {q_minimo:.4f}" if q_minimo is not None else "Q MÍNIMO: --")
    print(f"Q MÁXIMO ENCONTRADO: {q_maximo:.4f}" if q_maximo is not None else "Q MÁXIMO: --")
    ...
    if resultados:
        ...
        print()
        print(
            f"⚠️ NENHUM JOGO DENTRO DO Q {Q_MIN:.2f} → {Q_MAX:.2f}"
        )
        print(
            "🔬 O diagnóstico acima mostra os Q calculados "
            "ANTES do filtro."
        )
